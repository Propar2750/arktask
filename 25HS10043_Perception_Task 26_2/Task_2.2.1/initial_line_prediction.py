...
# Load the binary image (output of Method 2: Otsu binarisation + cluster removal)
binary_image = cv2.imread(os.path.join(SCRIPT_DIR, 'method2_cleaned.jpg'), cv2.IMREAD_GRAYSCALE)

if binary_image is None:
    print("Error: Could not load 'method2_cleaned.jpg'. Run initial_kernel_filter.py first to generate it.")
else:
    ...
    # 4. Save results
    cv2.imwrite(os.path.join(SCRIPT_DIR, 'iron_man_lines_detected.jpg'), line_image)
    cv2.imwrite(os.path.join(SCRIPT_DIR, 'iron_man_lines_filled.jpg'), closed)
    cv2.imwrite(os.path.join(SCRIPT_DIR, 'iron_man_lines_thinned.jpg'), thinned)
